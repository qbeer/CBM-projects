...
\vspace{5mm}
\par I ran the signal generator to generate 10000 events of phi right in the center of the target. It is useful to analyze such data because with the results one can separate the facts from the fancy when experimenting later. With known $\Phi$ multiplicity and efficiencies it is possible to test theoretical forecast according the outcome.
...
\begin{lstlisting}[language=C++]
void run_mc_phi(TString inFile="Signal_phi_10000.txt", const char* setupName = "sis100_electron", Int_t nEvents = 10000)
{
  TString outFile = "sim_phi_10000.root";
  TString parFile = "param_phi_10000.root";
...
